...
function downloadSpecSheetPdf() {
  ...
  const green = [63, 232, 117];
  const dark = [15, 22, 17];
  const lightGrey = [245, 245, 245];
  const midGrey = [105, 105, 105];
  const lineGrey = [215, 215, 215];
  ...
  function addFooter() {
    ...
    const footerText = "Generated by Battery Cell Calculator | Preliminary specification sheet";
  ...
  function checkPageSpace(required = 18) {
  ...
  function addHeader() {
    ...
    doc.setFontSize(18);
    doc.text("Battery Pack Specification Sheet", margin, 15);

    doc.setTextColor(230, 230, 230);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.text("Volt Energy Systems", margin, 23);

    doc.setTextColor(230, 230, 230);
    doc.text(`Generated: ${new Date().toLocaleDateString()}`, pageWidth - margin, 15, { align: "right" });
    doc.text("Status: Calculated estimate", pageWidth - margin, 23, { align: "right" });
  ...
  function addNote(textValue) {
    ...
    const wrapped = doc.splitTextToSize(textValue, pageWidth - margin * 2);
  ...
  addSectionTitle("Pack Overview");
  addRow("Pack configuration", `${lastResults.seriesCount}S${lastResults.parallelCount}P`);
  addRow("Total cell count", safe(lastResults.numberOfCells, "", 0));
  ...
  addSectionTitle("Pack Current & Power Limits");
  addRow("Maximum discharge current", safe(lastResults.maxDischargeCurrentA, " A", 0));
  addRow("Continuous discharge current", safe(lastResults.continuousDischargeCurrentA, " A", 0));
  addRow("Maximum charge current", safe(lastResults.maxChargeCurrentA, " A", 0));
  addRow("Maximum discharge power", safe(lastResults.maxDischargePowerKW, " kW", 1));
  addRow("Continuous discharge power", safe(lastResults.continuousDischargePowerKW, " kW", 1));
  addRow("Maximum charge power", safe(lastResults.maxChargePowerKW, " kW", 1));

  addSectionTitle("Cell Specification");
  ...
  addRow("Maximum discharge current", safe(lastResults.cellMaxDischargeCurrentA, " A", 1));
  addRow("Continuous discharge current", safe(lastResults.cellContinuousDischargeCurrentA, " A", 1));
  addRow("Maximum charge current", safe(lastResults.cellMaxChargeCurrentA, " A", 1));
  addRow("Cell weight", safe(lastResults.cellWeightG, " g", 1));

  addSectionTitle("Module 1 Specification");
  addRow("Module configuration", text(lastResults.module1Config));
  ...
  addRow("Module maximum discharge", safe(lastResults.moduleMaxDischargeCurrentA, " A", 0));
  addRow("Module continuous discharge", safe(lastResults.moduleContinuousDischargeCurrentA, " A", 0));
  addRow("Module maximum charge", safe(lastResults.moduleMaxChargeCurrentA, " A", 0));

  if (lastResults.hasSecondModule) {
    addSectionTitle("Module 2 Specification");
    addRow("Module configuration", text(lastResults.module2Config));
    ...
    addRow("Module maximum discharge", safe(lastResults.module2MaxDischargeCurrentA, " A", 0));
    addRow("Module continuous discharge", safe(lastResults.module2ContinuousDischargeCurrentA, " A", 0));
    addRow("Module maximum charge", safe(lastResults.module2MaxChargeCurrentA, " A", 0));
  }

  addSectionTitle("Vehicle Simulation");
  ...
  addRow("Drive cycle", text(lastResults.driveCycleLabel));
  addRow("Vehicle mass", safe(lastResults.vehicleMassKg, " kg", 0));
  addRow("Accessory load", safe(lastResults.assumedLoadKW, " kW", 2));
  addRow("Regenerative braking recovered", safe(lastResults.regenRecoveredKWh, " kWh", 2));

  addSectionTitle("Assumptions & Notes");
  addNote("This specification sheet is generated from the input values entered into the Battery Cell Calculator.");
  addNote("All figures are calculated estimates and should be reviewed before being used as a formal engineering specification.");
  addNote("Electrical, thermal, mechanical, safety, validation and compliance checks are still required before pack release or production use.");
